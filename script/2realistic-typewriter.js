...
(function () {
    ...
    for (row = 0; row < keyboardLayout.length; row++) {
      for (col = 0; col < keyboardLayout[row].length; col++) {
        if (keyboardLayout[row][col].toLowerCase() === character) {
          ...
          if (
            adjacentCol >= keyboardLayout[adjacentRow].length || adjacentCol < 0
          ) {
   ...
   */
  function createCharacterGenerator(
    text,
    keyboardLayout,
    accuracy,
    checkInterval,
  ...
  RealisticTypewriter.prototype.type = function (text, htmlElement, callback) {
    var self = this,
      checkInterval =
        (this.maximumCharactersPerSecond + this.minimumCharactersPerSecond) / 2,
      nextChar =
    ...
    if (
      htmlElement.removeChild === undefined ||
      htmlElement.lastChild === undefined ||
    ...
    function writeAndCallback(internalCallback) {
      var wait = randomInRange(
          1000 / self.minimumCharactersPerSecond,
          1000 / self.maximumCharactersPerSecond,
        ),
        ch = nextChar();
